...
namespace Core\Services\DevDumper\Caster;
...
class LinkStub extends ConstStub
{
    ...
    public function __construct(?string $label, int $line = 0, ?string $href = null)
    {
        ...
        if ($composerRoot = $this->getComposerRoot($href, $this->inVendor)) {
            ...
            $this->attr['ellipsis-tail'] = 1 + ($this->inVendor
                    ? 2 + \strlen(implode('', \array_slice(explode(\DIRECTORY_SEPARATOR, substr($href, 1 - $this->attr['ellipsis'])), 0, 2)))
                    : 0);
